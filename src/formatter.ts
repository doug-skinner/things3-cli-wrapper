 ...
 */
const color = {
  taskName: (text: string) => shouldDisableColors() ? text : chalk.white(text),
  ...
  dueDateOverdue: (text: string) => shouldDisableColors() ? text : chalk.red(text),
  dueDateSoon: (text: string) => shouldDisableColors() ? text : chalk.yellow(text),
  dueDateFuture: (text: string) => shouldDisableColors() ? text : chalk.white(text),
...
export function displayError(message: string): void {
  const errorText = shouldDisableColors() ? `Error: ${message}` : chalk.red(`Error: ${message}`);
